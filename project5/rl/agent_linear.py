...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.binomial(1,epsilon):
      ...
      action_index, object_index = np.unravel_index(np.argmax(q_value),(NUM_ACTIONS,NUM_OBJECTS))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    theta[tuple2index(action_index,object_index)] += ALPHA*(reward + GAMMA*maxQ - \
                                              q_value[tuple2index(action_index,object_index)])*current_state_vector
# pragma: coderesponse end
