...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.binomial(1,epsilon):
      ...
      action_index, object_index = torch.argmax(action_index),torch.argmax(object_index)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    if terminal:
      ...
      maxQ = maxq_next
    loss = 0.5*(reward + GAMMA*maxQ - q_current)**2
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            deep_q_learning(current_state_vector, next_action_index, next_object_index, reward,
                            next_state_vector, terminal)
